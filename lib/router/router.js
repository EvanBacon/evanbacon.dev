...
AlbumController = RouteController.extend({
    ...
    // onAfterAction: function () {
    //   // if this is first time on page, scroll to top
    //   if (Meteor.isClient) {
    //     var albumTitle = '',
    //         album = Albums.findOne({slug: this.params.slug}); 
    //     if ( !! album && !! album.title)
    ...
    //       console.log('scrolling to top of main');
    //       routerFilters.resetScroll();
    //   } 
...
TagMediaController = RouteController.extend({
    template: 'album',
    increment: getSetting('mediaPerPage', 30),
    limit: function() { 
      return parseInt(this.params.limit) || this.increment; 
...
TagListController = RouteController.extend({
  ...
  Router.configure({
    ...
    Router.onAfterAction(routerFilters.resetScroll, { except: ['mediaManager', 'album', 'tag']}); 
