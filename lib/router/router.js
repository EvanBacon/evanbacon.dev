...
AlbumGroupsController = RouteController.extend({
    waitOn: function () {
      return [ Meteor.subscribe('works'), Meteor.subscribe('albumListMedia') ];
    },
  });
...
AlbumListController = RouteController.extend({
    ...
    metadata: function() {
      return { title: 'Works', description: null};
    },
    items: function () {
      var keyword = '',
            query = '',
    ...
    subscriptions: function() {
      this.itemsSub = Meteor.subscribe('works', this.findOptions());
    },
    data: function() {
        return {
            items: this.items(),
  ...
  AtelierController = RouteController.extend({
      template: 'atelier',
      findOptions: function() {
        return { sort: { order: 1 }};
      },
      metadata: function() {
        return { title: 'Atelier', description: null};
      },
      items: function () {
        var keyword = '',
              query = '',
...
// Routes
  ...
  /**
    ...
    * Default page for viewing, adding works
    */
