...
TagListController = RouteController.extend({
    template: 'tagManager',
    metadata: function() {
      return { title: 'Tags', description: 'Photo gallery built with Meteor.js'}
    },
    tags: function () {
      var sortBy = SortAction.getSortBy(),
          sortObj;
...
Router.configure({
      ...
      onAfterAction: function() {
        ...
        if (typeof this.metadata() !== undefined && !! this.metadata()) {
            ...
            $('title').text(getSetting('title') + ' :: ' + meta.title);
            $('meta[name=description]').attr('content', meta.description);
        } else {
            $('title').text(getSetting('title'));
            $('meta[name=description]').attr('content', getSetting('siteDescription'));
        }
      }
    });
