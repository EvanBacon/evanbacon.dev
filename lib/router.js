    ...
    items: function () {
	  ...
	  waitOn: function () {
	    return [ Meteor.subscribe('media', this.findOptions()), Meteor.subscribe('albums') ];
	  },
	  data: function() {  
  ...
  AlbumEditController = RouteController.extend({
    ...
    onBeforeAction: function () {
        pageChanged(false); // reset page changed status
        this.next();
    },
    waitOn: function () {
  ...
  AlbumListController = RouteController.extend({
    ...
    items: function () {
        ...
        query = new RegExp( keyword, 'i' ),
        results;
        //pageType = Router.current().route.getName().replace("Manager", "");
  ...
  AlbumController = RouteController.extend({
    ...
    // },
    waitOn: function () {
      return [ Meteor.subscribe('album', this.params._id), Meteor.subscribe('albumMedia', this.params._id) ];
    },
    data: function() {  
        //var hasMore = this.items().count() === this.limit();       
        return {   
...
Router.configure({
    ...
    */
    Router.route('/admin/album-edit/:_id', {
      name          : 'albumEdit',
      path          : '/admin/album-edit/:_id',
