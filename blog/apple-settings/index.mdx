<Title date="2023-08-22T17:36:02.536Z">Custom Apple Settings UI with Expo</Title>

_Generating an iOS [Settings Bundle][apple-docs] and sharing settings with your Expo app._
...
![](./images/settings-spotlight.avif)

### Desktop Support

If your iOS app is available in the Mac App Store, then your settings will also be available in the native settings for the app when you press <Kbd>⌘,</Kbd>.

![](./images/mac-settings.avif)

## The Goal

Many years ago, I built a small video game, [**Pillar Valley**](https://pillarvalley.expo.app/), that I now pump full of whatever native functionality I stumble across. It has haptics, native navigation controllers, full universal linking, quick notes, Apple Handoff, password sharing, custom app icons, runs on iOS, Android and web, and is optimized for start-up by using Hermes (no JavaScript runs on-device on native platforms). All the features you expect when downloading a single-tap video game from the store.
...
![Custom native settings controls in the iOS Pillar Valley app](./images/main-settings.avif)
...
npx json2ts -i ./schema.json -o ./src/schema.d.ts --additionalProperties=false --unknownAny=false
```
...
For the more static data, I added a [hook that runs at startup](https://github.com/EvanBacon/pillar-valley/blob/fba10cbedd593e4976ad4d1aeada1a5834f1100e/src/hooks/useUpdatesInAppleSettings.ts#L14) in the application to sync info to the system settings. This is where I'd add the remote config info.
...
![Runtime settings in the iOS Pillar Valley app](./images/runtime-settings.avif)
...
```term
# Licenses generation command
npx npm-license-crawler --onlyDirectDependencies --dependencies --json src/constants/Licenses.json
```
...
![Licenses settings page in Pillar Valley](./images/licenses-settings.avif)
...
- Example: [Settings plugin in **Pillar Valley**](https://github.com/EvanBacon/pillar-valley/blob/fba10cbedd593e4976ad4d1aeada1a5834f1100e/app.config.ts#L39-L166).
- [`@config-plugins/apple-settings`](https://github.com/expo/config-plugins/tree/main/packages/apple-settings)
- Download [**Pillar Valley** in the iOS App Store](https://apps.apple.com/us/app/pillar-valley/id1336398804) to see the settings in action.
...
[apple-settings-json]: https://github.com/expo/config-plugins/blob/b90eed0c0c749c5e87227eb59eba4509ddd0528c/packages/apple-settings/src/schema/SettingsPlist.json
