...
This RFC proposes the introduction of API Routes (aka server functions) into the React Native ecosystem as part of [Expo Router](https://docs.expo.dev/routing/introduction/) v3 and the upcoming Expo SDK 50. The goal is to enable developers to write server-side logic directly in their React Native (iOS, Android, web) projects in a simple and developer-friendly manner. This feature aims to simplify server-side development, foster better security practices, and improve the developer experience.
...
The API Routes will be served at `http://localhost:8081/generate` with `npx expo` and can be used by making a request:
...
The project babel config is used to transpile the API routes. Indication is passed to the Babel caller via the `isServer` boolean. This can be used to change the preset based on the environment.
...
New request and response objects, [`ExpoRequest`][wcg-request] and [`ExpoResponse`][wcg-response], will be introduced. They will be based on the WinterCG specification and include additional properties to make them compatible with Expo Router. These are inspired by Remix, SvelteKit, and Next.js for simplicity.
...
In preperation for this feature, we've also built-in first-class support for [debugging network requests](https://docs.expo.dev/debugging/tools/) in Expo SDK 49 using Chrome DevTools.
